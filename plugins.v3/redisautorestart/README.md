...
MoviePilot V3 插件，用于检测 Redis 异常并按配置自动重启 MoviePilot。
...
1.0.1通过稳定SDK读取配置。未手工填写Redis URL时，仅在`CACHE_BACKEND_TYPE=redis`时使用`CACHE_BACKEND_URL`；使用默认cachetools缓存时跳过自动Redis检测，避免把未启用Redis误认为故障。手工指定Redis URL仍优先。

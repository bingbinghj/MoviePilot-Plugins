...
MoviePilot V3 插件，用于对多个 New API 站点执行每日签到。
...
1.0.12使用`app.sdk`的插件、事件及日志接口；浏览器访问通过`app.sdk.browser.launch_browser_context`启动CloakBrowser，上下文在成功或异常后都会关闭。原有账号配置、API签到和每日调度继续沿用。

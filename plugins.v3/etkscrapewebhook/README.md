...
MoviePilot V3专用插件。它会按媒体根目录合并重复的 `MetadataScrape`，等待同一媒体本批整理任务全部结束，再执行MoviePilot基础刮削；MoviePilot生成NFO并通过输出校验后，再通知ETK执行后置增强。

继承1.0.8的整批处理修复，插件通过 `TransferChain.get_queue_tasks()` 按媒体类型、媒体来源及来源内ID检查整理状态。只要还有等待或运行中的文件，就继续合并请求，包括同一部剧的多个季。这里等待的是当前已加入整理任务的文件，不等待尚未下载的分集。已完成、失败和其他作品的任务不会阻塞本批处理；读取整理状态失败时保留批次并重试。
...
1.0.9使用V3的`ScrapingChain`和稳定SDK；整理匹配包含媒体来源与来源内ID，TMDb主身份转换为ETK需要的`tmdb_id`，保留`episode_group`。音乐及没有TMDb身份的其他来源保留MoviePilot原生刮削，不提交ETK影视增强。日志使用V3当前写入器，不再访问已移除的`LoggerManager._file_handler`。
